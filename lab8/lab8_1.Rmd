...
penguins %>% 
  ...
  select(bill_length_mm, bill_depyh_mm, flipper_length_mm, Body_mass_g)%>%
  summarize(mean_body_mass_g=mean(body_mass_g, na.rm=T),mean_body_mass_g=mean(body_mass_g, na.rm=T),
```
...
penguins%>%
  group_by(species)%>%
  summarizemean_body_mass_g=mean(body_mass_g, na.rm=T)
```
