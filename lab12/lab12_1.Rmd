...
deserts%>%
  ...
  summarize(n=n(), .groups='keep')%>% #keep cancel the eeror 
...
theme(axis.text.x=element_text(angle=30, hjust = 2))+
  labs(title="ABABABABABBABABAAAAAA", x="Year", y="n")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "tomato4", fill = "sienna3" , bins=200)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="lemonchiffon1" , alpha  =0.4, color = "tomato4")+ #alpha is the transparency
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(fill="lemonchiffon1" , alpha  =0.4, color = "tomato4")+ #alpha is the transparency
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(aes(y = after_stat(density), fill = "deepskyblue4", alpha = 0.4, color = "black")+
  geom_density(color = "red")+
  labs(title = "Distribution of something")
```
...
homerange%>%
  select(log10.mass)%>%
summarize(min=min(log10.mass),max=max(log10.mass))
```
...
```{r}
homerange%>%
  mutate(mass_category=case_when(log10.mass<=3.65~"small",
         log10.mass>3.65 & log10.mass<=6.02~"medium", 
         log10.mass>6.02~"large")) %>%
ggplot(aes(x=mass_category, fill=class)) +
  geom_bar(position="dodge", alpha  =0.4, color = "tomato4")
