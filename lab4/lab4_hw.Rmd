...
```{r}
limited_data<-select(homerange, mean.mass.g, log10.mass, family, genus, species)
limited_data

```
```{r}
```{r}
owls <-filter(animaldata, family == "cervidae")
owls

```
```{r}
max(owls$mean.mass.g)
```
I can't find the owls family in the dataset
...
bild <- filter(homerange, taxon == "bild")
bild
```
```
I can't define the bird cataglory
