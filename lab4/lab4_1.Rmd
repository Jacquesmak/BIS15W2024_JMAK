...
mammal_data <- read_csv("data/mammal_lifehistories_v2.csv")
getwd()
```
...
```{r}
 rename(genus = Genus, 
         ...
         litter_per_year = `litter/year`)
```

```{r}
newnamemammal<-rename(mammal_data, genus="Genus", wean_mass="wean mass", max_life="max. life", litter_size="litter size", letter_per_year="litters/year")
```
...
```{r}
select(newnamemammal, "genus", "species", "mass")
```
