...
The data are tidy per tidyverse definition as all values has its own cell per column with level . The data are in long format. 
...
```{r}
 sydneybeaches_long_dateseperate<-sydneybeaches_long %>% 
...
```{r}
sydneybeaches_yearly_means_long <-  sydneybeaches_long_dateseperate %>%
  group_by(site, year) %>%
  summarize(mean = mean(enterococci_cfu_100ml,  na.rm = T)) %>%
  arrange(desc(mean))
sydneybeaches_yearly_means_long
```
...
sydneybeaches_yearly_means_long %>%
  ...
  arrange(-mean)
```
The little Bay beach is the most polluted beach
