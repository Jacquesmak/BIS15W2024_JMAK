...
vector is a series of data organized.

2. What is a data matrix in R?  
Data matrix are series of stacked vector
...
```{r}
hot_spring_matrix <- matrix(box_office, nrow = 8, byrow = T)
hot_spring_matrix
```
...
```{r}
mean <- rowMeans(hot_spring_matrix)
mean
```
...
```{r}
all_hot_spring_matrix <- cbind(hot_spring_matrix, mean)
...
```{r}
Jill_sum <- all_hot_spring_matrix[ ,1]
sum(Jill_sum)
```
